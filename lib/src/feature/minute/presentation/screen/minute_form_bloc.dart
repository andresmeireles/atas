...
class MinuteForm extends StatelessWidget {
    ...
    return AppScaffold(
      appBar: AppBar(
        ...
        actions: mutateState.status == MinuteBlocStatus.submitting
            ? [
                const Center(child: Padding(padding: EdgeInsets.only(right: 15.0), child: CircularProgressIndicator())),
              ]
  ...
  _showDialog(BuildContext context, MinuteBloc bloc) async {
    final meetItem = await showDialog<MeetItem?>(
      context: context,
      builder: (context) => BlocProvider<MinuteBloc>.value(value: bloc, child: const OptionDeciderDialogBloc()),
    );

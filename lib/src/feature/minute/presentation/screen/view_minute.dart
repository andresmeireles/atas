...
class _ViewMinuteState extends State<ViewMinute> {
  ...
  Assign? _getSingle(Label label, Types type) {
    final results = _minute.assignments.where((e) => e.label == label && e.type == type);
    if (results.isEmpty) return null;
    return results.first;
  }

  Call? _getSingleCall(Label label) => _getSingle(label, Types.call) as Call?;

  Hymn? _getSingleHymn(Label label) => _getSingle(label, Types.hymn) as Hymn?;

  SimpleText? _getSingleSimple(Label label) => _getSingle(label, Types.simpleText) as SimpleText?;
}
